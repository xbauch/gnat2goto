...
--  Precondition commented out because full extend of declrations not yet known
   ...
   --  We return and expression equivalent to:
   --    memcpy(dest.data,
   --           concat_expr(src_1, src_2, .., src_n).data,
   --           (dest.last - dest.first + 1) * sizeof(T));
   function Do_Array_Assignment (N : Node_Id) return Irep
   is
      --  We assume the lhs is allocated
      LHS_Node : constant Node_Id := Name (N);
      Source_Loc : constant Source_Ptr := Sloc (N);
      LHS_Expr : constant Irep := Do_Expression (LHS_Node);
      RHS_Expr : constant Irep := Do_Op_Concat (Expression (N));
      ...
      Void_Ptr_Type : constant Irep :=
           Make_Pointer_Type (Make_Void_Type, Pointer_Type_Width);
      ...
      Dest_Data : constant Irep :=
           Make_Member_Expr (Compound         => LHS_Expr,
                             Source_Location  => Source_Loc,
                             Component_Number => 2,
                             I_Type           => PElement_Type,
                             Component_Name   => "data");
      Source_Data : constant Irep :=
           Make_Member_Expr (Compound         => RHS_Expr,
                             Source_Location  => Source_Loc,
                             Component_Number => 2,
                             I_Type           => PElement_Type,
                             Component_Name   => "data");
      Dest_Num_Elems : constant Irep :=
        Build_Array_Size (Array_Comp => LHS_Expr,
                          Idx_Type   => Index_Type);

      Memcpy_Call : constant Irep :=
        Make_Memcpy_Function_Call_Expr (Destination       => Dest_Data,
                                        Source            => Source_Data,
                                        Num_Elem          => Dest_Num_Elems,
                                        Element_Type_Size => EType_Size,
                                        Source_Loc        => Source_Loc);
   begin
      return Make_Code_Assign (Rhs             => Memcpy_Call,
                               Lhs             => Memcpy_Lhs,
                               Source_Location => Source_Loc);
   ...
   is
      ...
      is
         ...
         Dest_Alloc : constant Irep :=
           Typecast_If_Necessary (Expr     => Raw_Dest_Alloc,
                                 New_Type => Make_Pointer_Type (Element_Type));
      begin
         ...
         Append_Op (Result_Block,
                    Make_Code_Return (Return_Value    => Dest_Symbol,
                                      Source_Location => Source_Loc,
                                      I_Type          => Result_Type));
         return Result_Block;
